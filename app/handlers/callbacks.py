...
async def route(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action not in ("del", "nop") and chat["menu_msg_id"] not in (None, query.message.message_id):
        await query.answer(texts.STALE_MENU, show_alert=False)
        await _render_fresh(context, chat_id)
        return
    ...
    if handler is None:
        ...
        return
    await handler(query, context, chat_id, parts[1:])
...
async def _render_fresh(context, chat_id: int) -> None:
    ...
    if step == "confirm":
        text, markup = keyboards.render_confirm(f)
    elif step in keyboards.WIZARD_SCREENS:
        text, markup = keyboards.render_screen(step, f, wizard=True)
    else:
        text, markup = keyboards.render_root(f)
    await _ui.send_menu(context.bot, chat_id, text, markup)


async def _show_screen(query, chat_id: int, screen: str) -> None:
...
async def _on_menu(query, context, chat_id, args) -> None:
    ...
    if target == "root":
        ...
        await _ui.edit_menu(query, text, markup)
        return
    await _show_screen(query, chat_id, target)


async def _on_wizard(query, context, chat_id, args) -> None:
    ...
    if move == "start":
        ...
        text, markup = keyboards.render_screen(
            keyboards.WIZARD_SCREENS[0], db.get_filter(chat_id), wizard=True
        )
        await _ui.edit_menu(query, text, markup)
        return
    ...
    if step not in keyboards.WIZARD_SCREENS:
        await _on_menu(query, context, chat_id, ["root"])
        return
    ...
    if idx >= len(keyboards.WIZARD_SCREENS):
        ...
        text, markup = keyboards.render_confirm(db.get_filter(chat_id))
        await _ui.edit_menu(query, text, markup)
        return

    await _show_screen(query, chat_id, keyboards.WIZARD_SCREENS[idx])


async def _on_set(query, context, chat_id, args) -> None:
    key, raw = args[0], args[1]
    if key == "wbs":
        db.update_filter(chat_id, {"wbs_required": "" if raw == keyboards.ANY else raw})
        await _show_screen(query, chat_id, "wbs")
        return
    ...
    await _show_screen(query, chat_id, _SCREEN_OF[key])
...
async def _on_ask(query, context, chat_id, args) -> None:
    key = args[0]
    field = keyboards.ASK_FIELDS[key]
    db.set_chat(chat_id, awaiting=field)
    await _ui.edit_menu(query, texts.ASK_NUMBER[field] + texts.ASK_HINT)


async def _on_toggle(query, context, chat_id, args) -> None:
    ...
    if kind == "d":
        ...
        await _show_screen(query, chat_id, "dist")
    else:
        ...
        await _show_screen(query, chat_id, "prov")


async def _on_all(query, context, chat_id, args) -> None:
    kind = args[0]
    if kind == "d":
...
async def _on_filter_action(query, context, chat_id, args) -> None:
    what = args[0]
    if what == "clear":
        db.clear_filter(chat_id)
        text, markup = keyboards.render_root(db.get_filter(chat_id))
        await _ui.edit_menu(query, f"{texts.FILTER_CLEARED}\n\n{text}", markup)
        return

    if what == "help":
        await _ui.edit_menu(query, texts.HELP)
        return
    ...
    if what in ("activate", "done"):
        ...
        if chat["state"] in ("new", "setup"):
            ...
            await _ui.edit_menu(query, texts.SETUP_DONE.format(
                summary=formatting.filter_summary(db.get_filter(chat_id))
            ))
        else:
            db.set_chat(chat_id, setup_step="", awaiting="")
            await _ui.edit_menu(query, texts.MENU_TITLE.format(
                summary=formatting.filter_summary(db.get_filter(chat_id))
            ))


async def _on_delete(query, context, chat_id, args) -> None:
    if args and args[0] == "yes":
        db.delete_chat(chat_id)
        await _ui.edit_menu(query, texts.DELETED)
    else:
        await _ui.edit_menu(query, texts.DELETE_ABORTED)


async def _on_nop(query, context, chat_id, args) -> None:
    return
